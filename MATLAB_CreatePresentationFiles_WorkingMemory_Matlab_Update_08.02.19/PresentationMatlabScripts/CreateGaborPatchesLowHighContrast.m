function CreateGaborPatchesLowHighContrast()
    ...
    strOutputFolder = '/Users/mmb/MEG/ATWM1/Presentation/StimuliProPixx/';
 
    vPhases = [0.25, 0.25];
    %vPhases = [0.25];
    
    dbScale = getScalingFactorOldProjectorToProPixx();
        ...
        for bAddFrame = [0 1]
            ...
            for iPhase = 1:length(vPhases)
                ...
                for iAngle = 1:180
                    ...
                    %% parameters
                    imSize = floor(65 * dbScale);    % image size: n X n
                    lamda = 25 * dbScale;   %15                          % wavelength (number of pixels per cycle)
                    theta = iAngle;                         % grating orientation
                    sigma = 12 * dbScale;    %10                         % gaussian standard deviation in pixels
                    ...
                    if bCircularGrating
                       strOutputPath = sprintf('%sgabor_circ.bmp', strOutputFolder);     
                    else
                        if iPhase == 1
                            if bAddFrame
                            ...
                            if bAddFrame                                
                                strOutputPath = sprintf('%sgabor_%03d_alt_framed.bmp', strOutputFolder, iAngle);
                            else
                    ...
                    if bAddFrame
                        ...
                        iPixelThickness = ceil( 3 * dbScale );
